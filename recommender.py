...
class Recommender:
    def train(self, prices, database) -> None:
        ...
        def generate_association_rules(frequent_itemsets, min_confidence):
            rules = []
            for itemset, rsup in frequent_itemsets:
                if len(itemset) > 1:
                    for i in range(1, len(itemset)):
                        for antecedent in itertools.combinations(itemset, i):
                            consequent = tuple(sorted(set(itemset) - set(antecedent)))
                            if consequent:
                                antecedent_support = get_rsup(frequent_itemsets, antecedent)
                                confidence = rsup / antecedent_support
                                ...
                                if confidence >= min_confidence and lift > 1:
                                    ...
                                    rules.append((antecedent, consequent, profits, confidence, lift, conviction, combined_metric, rsup))
        ...
        def get_rsup(frequent_itemsets, itemset):
            ...
            for fi, rsup in frequent_itemsets:
        ...
        print(f"{'Itemset':<20} {'Relative Support':<10}")
        for itemset, rsup in self.frequent_itemsets:
            print(f"{str(itemset):<20} {rsup:<10.2f}")

        # Imprimir reglas de asociación
        print(f"{'Antecedent':<20} {'Consequent':<20} {'Profit':<10} {'Confidence':<10} {'Lift':<10} {'Conviction':<10} {'Confidence + Lift':<10} {'Relative Support':<10}")
        for rule in self.rules:
            antecedent, consequent, profits, confidence, lift, conviction, combined_metric, rsup = rule
            print(f"{str(antecedent):<20} {str(consequent):<20} {profits:<10.2f} {confidence:<10.2f} {lift:<10.2f} {conviction:<10.2f} {combined_metric:<10.2f} {rsup:<10.2f}")
